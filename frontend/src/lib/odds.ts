...
export function transformEvent(event: OddsEvent, league: string): GameCard | null {
  try {
    ...
    const confidence = Math.min(0.5 + event.bookmakers.length * 0.03, 0.82);
  ...
  for (const game of games) {
    ...
    if (p > 0.65 && game.spread < 0) {
      ...
      picks.push({
        ...
        reasoning: `Model assigns ${Math.round(p * 100)}% home win probability. Spread edge detected vs market-implied ${Math.round((p - edge) * 100)}%.`,
        hot: edge >= 0.08,
      });
    } else if (p < 0.38) {
      const awayProb = 1 - p;
      ...
      picks.push({
        ...
        edge,
        confidence: game.confidence,
        reasoning: `Away team assigned ${Math.round(awayProb * 100)}% win probability. Market undervaluing road side at current line.`,
        hot: edge >= 0.08,
      });
    }
